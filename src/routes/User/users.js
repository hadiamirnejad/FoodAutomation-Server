...
const signInRoute = {
  ...
  handler: async (req, res) => {
    ...
    if(userCount===0){
      ...
      const token = await bcrypt.hash(result._doc.username + result.password + result.role + process.env.JWT_SECRET, 10);

      await Token.deleteMany({user: result._id});
      await Token.create({token: token, user: result._id});
      const { password, ...userD } = result._doc;
      return res.status(200).json({ accessToken: token, user: userD});
    }
...
const addUser = {
  ...
  checkTokenValidation: false,
  handler: async (req, res) => {
    const { username, name, role, password, unit } = req.body;
    ...
    try{
      ...
      result = await User.findById(result._doc._id).populate({path: 'unit', select: {'_id': 1, 'title': 1}})
...
const editUser = {
  ...
  checkTokenValidation: false,
  handler: async (req, res) => {
    const { username, name, role, unit, id } = req.body;
    ...
    const result = await User.findByIdAndUpdate(id,{
      ...
      role,
      unit
    }, {new: true}).populate({path: 'unit', select: {'_id': 1, 'title': 1}});

    res.status(200).json({updatedUser: {_id: result._id, username: result.username, active: result.active, name: result.name, avatar: result.avatar, unit: result.unit, role: result.role, currentFile: result.currentFile}})
  },
}
...
const getUsers = {
  ...
  handler: async (req, res) => {
    ...
    const users = await User.find(search).populate({path: 'unit', select: {'_id': 1, 'title': 1}});

    return res.json({users: users.map(u=> {return {_id: u._id, username: u.username, active: u.active, name: u.name, avatar: u.avatar,unit: u.unit, role: u.role, currentFile: u.currentFile}})})
  },
}
...
const deactiveUser = {
  ...
  handler: async (req, res) => {
    ...
    res.status(200).json({updatedUser: {_id: result._id, username: result.username, active: result.active, name: result.name, avatar: result.avatar, unit: result.unit, role: result.role, currentFile: result.currentFile}}, { csrfToken: req.csrfToken() })
  },
};
